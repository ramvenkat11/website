...
running the code we ship, with the settings it ships with. Nothing is modelled or extrapolated.
...
To know how well it does that, we need catalogues of agents and questions with a known right
answer — so we built five, each a realistic working catalogue for a different kind of business,
plus one very large catalogue to test what happens at scale.

| catalogue | agents | questions asked |
|---|---|---|
| a university | 50 | 500 |
...
| **a catalogue spanning fifty business areas** | **1,000** | **10,000** |

**15,000 questions in total.** Every question has one known correct agent, and every question was
put through the real search.

The five business catalogues are deliberately different from one another in the way that matters
...
| catalogue | right agent first | right agent among the first three |
|---|---|---|
...
| a catalogue of 1,000 agents | 85.5% | 94.1% |

Two things are worth drawing out.

**The spread between catalogues is larger than the spread between any settings we tried.** The
easiest and hardest catalogues here are seven points apart, and both are 100-agent catalogues.
...
**Size is not the problem people expect it to be.** A catalogue of a thousand agents scores
within a point of a catalogue of a hundred. Adding agents does not make search worse; adding
...
asked of a university's catalogue. These are refused **100% of the time**.
...
because every agent in the catalogue is in the same neighbourhood and something always looks
close. They are refused **82% to 86% of the time**, depending on the catalogue.
...
A search compares the question against everything in the catalogue and returns the best matches
in **about 200 milliseconds**.

A catalogue of a thousand agents takes about the same — a little over 200 milliseconds. That is the number worth remembering: **search
does not get slower as your catalogue grows.**
...
non-English catalogue is within about three points of English, and asking across two different
...
Every number above comes from catalogues we built, and a real catalogue will differ. The honest
way to read them is as a range: a well-separated catalogue lands at the top of it, a catalogue
